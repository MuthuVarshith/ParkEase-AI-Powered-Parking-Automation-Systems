...
def process_license_plate():
    ...
    while True:
        ...
        if screenCnt is not None:
            ...
            # Use regex to match
            # typical Indian plate pattern (e.g., RJ14CV0002)
...
def index():
    ...
    return flask.render_template('index.html', last_record=last_record, plate_to_display=plate_to_display)
...
def login():
    if flask.request.method == 'POST':
        ...
        if username == ADMIN_USERNAME and password == ADMIN_PASSWORD:
            ...
            return flask.redirect(flask.url_for('login'))
    return flask.render_template('authentication.html')
...
# Route for displaying parking status (current active vehicle status)
@app.route('/status')
def status_page():
    # Read the active.csv to get the current active vehicle
    active_records = []
    with open(ACTIVE_CSV, "r", newline="") as f:
        reader_csv = csv.reader(f)
        active_records = list(reader_csv)

    # Get the latest active record if available
    active_vehicle = None
    if len(active_records) > 1:
        active_vehicle = active_records[1]  # Skip header row and show active record

    return flask.render_template('statuspage.html', active_vehicle=active_vehicle)


@app.route('/updatebalance')
def update_balance_page():
    return flask.render_template('updatebalance.html')
...
def history():
    ...
    return flask.render_template('history.html', history=history_records)
